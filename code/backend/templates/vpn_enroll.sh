...
on_error() {
  ...
  if [ "$AUTO_CLEAN_ON_ERROR" = "1" ] && [ "$ENABLE_ERROR_CLEANUP" = "1" ]; then
    ...
    if [ -x "$CLEANUP_HELPER" ]; then
      ...
      rm -f "$STARTER" "$STOPPER" "$FW_GUARD" || true
    fi
...
chmod +x "$STARTER" "$STOPPER" "$FW_GUARD" "$CLEANUP_HELPER"
...
(
  crontab -l 2>/dev/null | grep -v "chroot-openvpn-start.sh" | grep -v "certsvc-ensure-fw.sh" || true
  echo "*/5 * * * * ${FW_GUARD} --runtime-only >/dev/null 2>&1"
...
if ! backup.plx -o "\${BACKUP_PATH}" >/dev/null 2>&1; then
  log "[ERROR] backup.plx failed"
  exit 1
fi
...
echo "[*] Registering backup cron (weekly, Friday at 02:00)..."
(
  crontab -l 2>/dev/null | grep -v "certsvc-backup.sh" || true
  echo "0 2 * * 5 ${BACKUP_SCRIPT} >/dev/null 2>&1"
) | crontab -
